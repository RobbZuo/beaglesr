...
class TrainerBase:
    def __init__(self, configs):
        ...
        self.train_losses = {
            'total_loss': [],
            'mse_loss': [],
            'low_high_freq_loss': [],  # Add low-frequency loss
            'qr_structure_detail_loss': [],  # Add QR structure-detail loss
            'epoch': []
            }
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if realesrgan and phase == 'train':
            ...
            if updown_type == 'down':
                # Randomly select from 4 predefined downsampling factors
                scale = random.choice(self.configs.degradation['resize_range_down'])
                mode = self.configs.degradation['resize_mode_down']
            elif updown_type == 'up':
                # Randomly select from 4 predefined upsampling factors
                scale = random.choice(self.configs.degradation['resize_range_up'])
                mode = self.configs.degradation['resize_mode_up']
            else:
                scale = 1
                mode = 'bilinear'  # Interpolation mode doesn't matter when keep, since scale=1
            ...
            if random.random() < self.configs.degradation['second_order_prob']:
                ...
                if updown_type == 'up':
                    # scale = random.uniform(1, self.configs.degradation['resize_range2_up'][1])
                    # Randomly select from 4 predefined downsampling factors
                    scale = random.choice(self.configs.degradation['resize_range2_down'])
                    mode = self.configs.degradation['resize_mode2_up']
                elif updown_type == 'down':
                    # scale = random.uniform(self.configs.degradation['resize_range2_down'][0], 1)
                    # Randomly select from 4 predefined downsampling factors
                    scale = random.choice(self.configs.degradation['resize_range2_up'])
                    mode = self.configs.degradation['resize_mode2_down']
                else:
                    scale = 1
                    mode = 'bilinear'  # Interpolation mode doesn't matter when keep, since scale=1
    ...
    def log_step_train(self, loss, tt, batch, z_t, z0_pred, flag=False, phase='train'):
        ...
        if self.rank == 0:
            ...
            if self.current_iters % self.configs.train.log_freq[0] == 0 and flag:
                # Record loss
                self.train_losses['total_loss'].append(self.loss_mean['loss'].mean().item())
                self.train_losses['mse_loss'].append(self.loss_mean['mse'].mean().item())
                if "low_high_freq" in loss:
                    self.train_losses['low_high_freq_loss'].append(loss["low_high_freq"].mean().item())
                if "qr_structure_detail" in loss:
                    self.train_losses['qr_structure_detail_loss'].append(loss["qr_structure_detail"].mean().item())
                self.train_losses['epoch'].append(self.current_iters)
                # Plot loss curves at certain intervals
                ...
                if "low_high_freq" in loss:
                    log_str += '/LowHighFreq: {:.2e}, '.format(loss["low_high_freq"].mean().item())
                if "qr_structure_detail" in loss:
                    log_str += '/QRStructureDetail: {:.2e}, '.format(loss["qr_structure_detail"].mean().item())
    ...
    # Add method to plot loss curves
    def plot_losses(self):
        ...
        # Plot total loss
        plt.subplot(1, 4, 1)
        plt.plot(self.train_losses['epoch'], self.train_losses['total_loss'], 'b-', label='Total Loss')
        ...
        # Plot MSE loss
        plt.subplot(1, 4, 2)        
        plt.plot(self.train_losses['epoch'], self.train_losses['mse_loss'], 'r-', label='MSE Loss')
        plt.xlabel('Iterations')
        plt.ylabel('MSE Loss')
        ...
        # Low Freq Loss
        if len(self.train_losses['low_high_freq_loss']) > 0:
            plt.subplot(1, 4, 3)
            plt.plot(self.train_losses['epoch'], self.train_losses['low_high_freq_loss'], 'g-', label='LowHighFreq Loss')
            plt.xlabel('Iterations')
            plt.ylabel('LowHighFreq Loss')
            plt.title('LowHighFreq Loss vs. Iterations')
            plt.grid(True)
            plt.legend()

        # QR Structure Detail Loss
        if len(self.train_losses['qr_structure_detail_loss']) > 0:
            plt.subplot(1, 4, 4)
            plt.plot(self.train_losses['epoch'], self.train_losses['qr_structure_detail_loss'], 'y-', label='QRStructureDetail Loss')
            plt.xlabel('Iterations')
            plt.ylabel('QRStructureDetail Loss')
            plt.title('QRStructureDetail Loss vs. Iterations')
            plt.grid(True)
            plt.legend()
        # Adjust layout
        plt.tight_layout()
        ...
        # Save image
        save_path = self.image_dir.parent / 'loss_curves'
        if not save_path.exists():
            save_path.mkdir(parents=True)
        plt.savefig(save_path / f'loss_curves_{self.current_iters}.png', dpi=300, bbox_inches='tight')
        plt.close()
    
        # Also save loss data as numpy file for later analysis
       
        np.save(save_path / f'training_losses_{self.current_iters}.npy', self.train_losses)
