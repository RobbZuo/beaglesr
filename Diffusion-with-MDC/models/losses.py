...
def low_high_frequency_loss(x, y, cutoff_freq=0.3):
    """
    Frequency-domain loss with a Gaussian soft mask in the FFT domain.
    ...
    """
    ...
    y_coords, x_coords = th.meshgrid(
        th.arange(H, device=x.device),
        th.arange(W, device=x.device),
        indexing="ij",
    )
    dist_from_center = th.sqrt((y_coords - center_h) ** 2 + (x_coords - center_w) ** 2)
    freq_radius = int(min(H, W) * cutoff_freq / 2)
    sigma = freq_radius / 2.0
    low_mask = th.exp(-(dist_from_center ** 2) / (2 * sigma ** 2))
    ...
    for c in range(C):
        ...
        x_freq = th.fft.fftshift(th.fft.fft2(x_smooth.squeeze(1)))
        y_freq = th.fft.fftshift(th.fft.fft2(y_smooth.squeeze(1)))
