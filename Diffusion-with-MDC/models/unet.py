...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        #print(f"lq_size: {lq_size}")
        #print(f"cond_lq: {cond_lq}")

        if cond_lq and lq_size == image_size:
            self.feature_extractor = nn.Identity()
            # print(f"cond_lq and lq_size == image_size")
            base_chn = 3
            # base_chn = 16
        else:
            print(f"cond_lq and lq_size != image_size")
            ...
            base_chn = 16
            # base_chn = 32
            # for ii in range(int(math.log(lq_size / image_size) / math.log(2)) // 2):
