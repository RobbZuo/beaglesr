...
from .losses import normal_kl, discretized_gaussian_log_likelihood, low_high_frequency_loss, qr_structure_detail_loss
...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            if self.low_high_frequency_weight > 0:
                terms["low_high_freq"] = low_high_frequency_loss(pred_zstart, z_start)
            else:
                terms["low_high_freq"] = 0

            # QR structure-detail loss
            if self.qr_structure_detail_weight > 0:
                terms["qr_structure_detail"] = qr_structure_detail_loss(pred_zstart, z_start)
            else:
                terms["qr_structure_detail"] = 0
